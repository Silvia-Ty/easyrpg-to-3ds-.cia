...
def make_cia_file(id, title, author, release_date, game_path, name, safe_name, banner, audio, icon, elf, spec, tmp, out, rel_dir, verbose_mode=False):
  ...
  cmd = ['./bannertool/bannertool.exe', 'makebanner', '-i', banner, '-a', audio, '-o', tmp + '/banner.bin']
  print_verbose(' '.join(cmd), verbose=verbose_mode)
  result = subprocess.run(cmd, capture_output=True)
  if result.returncode != 0: return report_cia_error(1, 'makebanner', rel_dir)
  cmd = ['./bannertool/bannertool.exe', 'makesmdh', '-s', title, '-l', title, '-p', author, '-i', icon, '-o', tmp + '/icon.bin']
  print_verbose(' '.join(cmd), verbose=verbose_mode)
  result = subprocess.run(cmd, capture_output=True)
  if result.returncode != 0: return report_cia_error(2, 'makesmdh', rel_dir)
  cmd = ['./3dstool/3dstool.exe', '-cvtf', 'romfs', tmp + '/romfs.bin', '--romfs-dir', game_path]
  print_verbose(' '.join(cmd), verbose=verbose_mode)
  result = subprocess.run(cmd, capture_output=True)
  ...
  cmd = ['./makerom/makerom.exe', '-f', 'cia', '-o', out + '/' + safe_name + '.cia', '-elf', elf, '-rsf', tmp + '/spec.rsf', '-icon', tmp + '/icon.bin', '-banner', tmp + '/banner.bin', '-exefslogo', '-target', 't', '-romfs', tmp + '/romfs.bin']
  print_verbose(' '.join(cmd), verbose=verbose_mode)
  result = subprocess.run(cmd, capture_output=True)
...
def main():
  ...
  parser.add_argument('--elf', type=str, help='path to an EasyRPG ELF file', default='./elf/easyrpg-player.elf')
  parser.add_argument('--spec', type=str, help='path to a ROM spec file (will get a new unique ID)', default='./spec/spec.rsf')
  parser.add_argument('--rtp', type=str, help='path to the directory containing RTPs', default='./rtp')
...
def check_prerequisites():
  has_bannertool = bin_is_available('./bannertool/bannertool.exe')
  has_3dstool = bin_is_available('./3dstool/3dstool.exe')
  has_makerom = bin_is_available('./makerom/makerom.exe')
